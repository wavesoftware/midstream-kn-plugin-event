...
TMPDIR=$(mktemp -d openshift-kn-event-update-ci-XXXXX -p /tmp/)
readonly TMPDIR
cleanup() {
  rm -rf "${TMPDIR}"
}
...
CONFIGDIR=$OPENSHIFT/ci-operator/config/openshift-knative/pkg
test -d "$CONFIGDIR" || fail "'$CONFIGDIR' is not a directory"
PERIODIC_CONFIGDIR=$OPENSHIFT/ci-operator/jobs/openshift-knative/pkg
test -d "$PERIODIC_CONFIGDIR" || fail "'$PERIODIC_CONFIGDIR' is not a directory"

# Generate CI config files
CONFIG=$CONFIGDIR/openshift-knative-pkg-release-$VERSION
PERIODIC_CONFIG=$PERIODIC_CONFIGDIR/openshift-knative-pkg-release-$VERSION-periodics.yaml
CURDIR=$(dirname "$0")
...
for OCP_VERSION in 47; do
    sed -i "/  name: periodic-ci-openshift-knative-pkg-release-${VERSION}-${OCP_VERSION}-e2e-aws-ocp-${OCP_VERSION}-continuous\n  spec:/ r $TMPDIR/reporterConfig" "$PERIODIC_CONFIG"
done
